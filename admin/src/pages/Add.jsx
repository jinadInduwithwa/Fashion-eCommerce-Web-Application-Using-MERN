  ...
  return (
    <form className='flex flex-col w-full items-start gap-3' onSubmit={onSubmitHandler}>
      ...
      <div className='w-full'>
        ...
        <input type="text" onChange={(e) => setName(e.target.value)} value={name} className='w-full max-w-[500px] px-3 py-2' placeholder='Enter Product Name' />
      </div>

      <div className='w-full'>
        <p className='mb-2'>Product Description</p>
        <textarea onChange={(e) => setDescription(e.target.value)} value={description} className='w-full max-w-[500px] px-3 py-2' placeholder='Enter Description' />
      </div>
      ...
      <div className='flex flex-col sm:flex-row gap-2 w-full sm:gap-8'>
        ...
        <div>
          ...
          <input className='w-full px-3 py-2 sm:w-[120px]' onChange={(e) => setPrice(e.target.value)} value={price} type="number" placeholder='' />
        </div>
      </div>
      ...
      <div>
        ...
        <div className='flex gap-3'>
          <div onClick={() => setSizes(prev => prev.includes("S") ? prev.filter(item => item !== "S") : [...prev])}>
            <p className={`${sizes.includes("S") ? "bg-slate-600" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>S</p>
          </div>

          <div onClick={() => setSizes(prev => prev.includes("M") ? prev.filter(item => item !== "M") : [...prev])}>
            <p className={`${sizes.includes("M") ? "bg-slate-600" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>M</p>
          </div>

          <div onClick={() => setSizes(prev => prev.includes("L") ? prev.filter(item => item !== "L") : [...prev])}>
            <p className={`${sizes.includes("L") ? "bg-slate-600" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>L</p>
          </div>

          <div onClick={() => setSizes(prev => prev.includes("XL") ? prev.filter(item => item !== "XL") : [...prev])}>
            <p className={`${sizes.includes("XL") ? "bg-slate-600" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>XL</p>
          </div>

          <div onClick={() => setSizes(prev => prev.includes("XXL") ? prev.filter(item => item !== "XXL") : [...prev])}>
            <p className={`${sizes.includes("XXL") ? "bg-slate-600" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>XXL</p>
          </div>
        </div>
      </div>
